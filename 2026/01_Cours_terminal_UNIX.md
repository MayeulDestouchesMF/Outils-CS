...
footer: Outils pour le calcul scientifique 2026 - Introduction à UNIX -  Thibault Marzlin, Mayeul Destouches
---
...
Ce que vous avez appris avec `ls` (complétion, caractère générique, options) peut être appliqué à la plupart des commandes unix.
...
## Navigation
Pour changer de répertoire, il faut utiliser la commande `cd` (change directory).
`pwd` (print working directory) permet de savoir où vous êtes.
...
Si vous avez besoin de copier un répertoire entier, il faut explicitement mettre l'option `-r` (recursive):
...
# UNIX CLI part 4/4 :  Tester vos bases


---
# Quelques messages d'erreurs à interpréter
